...
void new_direction_raytracing (atmosphere_struct *atmos, char *datapath, double mu, double phi,
			  direction *dir, double phi_inc, float wvnmlo, float wvnmhi)
{
  ...
  if (first_call) {
    ...
    for (iprop=0; iprop<atmos->n_raytracing_prop; iprop++){
        if (strstr(atmos->raytracing_prop[iprop].name, "columns")){
            ...
            fprintf(stderr, "ERROR crystal name is '%s', but has to be either 'plates*' or 'columns*'!\n", atmos->raytracing_prop[iprop].name);
    }
    first_call=0;
  }
  ...
  dist = get_crystal_geometry(atmos, p1, p2, p3, p1_col, p2_col, p3_col, p1_pla, p2_pla, p3_pla, fraction_col, &deg_of_freedom, fraction_col_orient, fraction_pla_orient, dist_col, dist_pla, &crystal_axis_tilt, um ,vm);
