/*--------------------------------------------------------------------
 * $Id: albedo.c 3451 2019-01-28 00:28:05Z bernhard.mayer $
 * 
...
static int interpolate_rpv (rpv_out_struct *rpv, float *lambda, 
			    int nlambda_lower, int nlambda_upper);
static int read_RPV_spectral_BRDFs (char *filename, surfaces_out_struct *surf,
				    float *lambda, int nlambda,
				    int nlambda_lower, int nlambda_upper,
				    float sigma, float t1, float t2, float scale,
				    int quiet);
static int read_AMBRALS_spectral_BRDFs (char *filename, surfaces_out_struct *surf, float *lambda, int nlambda,
					int nlambda_lower, int nlambda_upper,
					int quiet);
...
/*      } */
  ...
  if (strlen(input.rte.mc.filename[FN_MC_RPV_SPECTRAL]) > 0) {
    ...
    status = read_RPV_spectral_BRDFs (input.rte.mc.filename[FN_MC_RPV_SPECTRAL], &(output->surfaces),
    ...
    if (status!=0) {
      fprintf (stderr, "Error %d returned by read_RPV_spectral_BRDFs from %s (line %d, function %s in %s)\n", 
  ...
  if (strlen(input.rte.mc.filename[FN_MC_AMBRALS_SPECTRAL]) > 0) {
    ...
    status = read_AMBRALS_spectral_BRDFs (input.rte.mc.filename[FN_MC_AMBRALS_SPECTRAL], &(output->surfaces),
					  output->wl.lambda_r, output->wl.nlambda_r,
					  output->wl.nlambda_rte_lower, output->wl.nlambda_rte_upper,
					  input.quiet);
    if (status!=0) {
      fprintf (stderr, "Error %d returned by read_AMBRALS_spectral_BRDFs from %s (line %d, function %s in %s)\n", 
                       status, input.rte.mc.filename[FN_MC_AMBRALS_SPECTRAL], __LINE__, __func__, __FILE__);
      return status;
    }
  }
  ...
  if (strlen(input.rte.mc.filename[FN_MC_ALBEDO_SPECTRAL]) > 0) { 
    ...
    status = read_spectral_albedos (input.rte.mc.filename[FN_MC_ALBEDO_SPECTRAL], &(output->surfaces),
				    output->wl.lambda_r, output->wl.nlambda_r,
				    output->wl.nlambda_rte_lower, output->wl.nlambda_rte_upper,
				    input.quiet);
    if (status!=0) {
...
static int read_RPV_spectral_BRDFs (char *filename, surfaces_out_struct *surf, float *lambda, int nlambda,
				    int nlambda_lower, int nlambda_upper,
				    float sigma, float t1, float t2, float scale,
				    int quiet) 
{
