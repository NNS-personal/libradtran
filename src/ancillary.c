/*--------------------------------------------------------------------
 * $Id: ancillary.c 3521 2019-12-11 22:28:30Z Claudia.Emde $
 * 
  ...
  if (tau_wvl>0){
    ...
    for(i_wvl=0; i_wvl<nwvl; i_wvl++){
      if( wvl_active[i_wvl] == 1 ){
        if( wvl_min_tmp > wvl[i_wvl] ) wvl_min_tmp=wvl[i_wvl];
        if( wvl_max_tmp < wvl[i_wvl] ) wvl_max_tmp=wvl[i_wvl];
      }
    }
  ...
  if (output->wl.use_reptran==0) {
    ...
    if (input.rte.solver == SOLVER_POLRADTRAN) {
      ...
      if (output->mc.sample.passback3D){
        ...
        if (input.rte.mc.jacobian)
          output->jacobian_t = calloc (output->atm.nzout, sizeof (float *****));
	...
	if (output->mc.sample.passback3D){
          ...
          if (input.rte.mc.jacobian)
            output->jacobian_t [lu] = calloc (output->mc.sample.Nx, sizeof (float *****)); 
	  ...
	  for (is=output->islower; is<=output->isupper; is++){
            ...
            if (input.rte.mc.jacobian)
              output->jacobian_t[lu][is] = calloc (output->mc.sample.Ny, sizeof (float ****));
	    ...
	    for (js=output->jslower; js<=output->jsupper; js++) {
              ...
              if (input.rte.mc.jacobian){
                output->jacobian_t [lu][is][js] = calloc ( input.n_caoth+2, sizeof (float ***));
                for (isp=0; isp<input.n_caoth+2; isp++){
                  output->jacobian_t [lu][is][js][isp] = calloc ( 2, sizeof (float **));
                  for (i=0; i<2; i++){
                    output->jacobian_t [lu][is][js][isp][i] = calloc ( output->atm.nlev-1, sizeof (float *));
                    
                    for (lc=0; lc<output->atm.nlev-1; lc++){
                      output->jacobian_t [lu][is][js][isp][i][lc] = calloc (output->wl.nlambda_t, sizeof (float));
                      status += weighting_reptran(&output->wl, 0, output->jacobian_r[lu][is][js][isp][i][lc], output->jacobian_t[lu][is][js][isp][i][lc]);
                    }
                  }
                }
              }
    ...
    for (iv=0; iv<output->wl.nlambda_t; iv++) {
      ...
      status = scale_output (input,
                             ...
                             &(output->uavgdn3d_t), &(output->uavgup3d_t), &(output->radiance3d_t),
                             &(output->jacobian_t), &(output->absback3d_t), 
                             &(output->rfldir3d_var_t), &(output->rfldn3d_var_t), &(output->flup3d_var_t),
                             &(output->uavgso3d_var_t),
                             &(output->uavgdn3d_var_t), &(output->uavgup3d_var_t),
                             &(output->radiance3d_var_t),  &(output->abs3d_var_t), &(output->absback3d_var_t), 
                             output->atm.nzout, output->atm.Nxcld, output->atm.Nycld, output->atm.Nzcld,  output->mc.alis.Nc, output->atm.nlev-1,
                             output->atm.threed, 
	      ...
	      status += arb_wvn(output->wl.nlambda_t, output->wl.lambda_t, output->uavgup3d_t[lu][is][js], 
              ...
              if (input.rte.mc.jacobian)
                for (isp=0; isp<input.n_caoth+2; isp++)
                  for (i=0; i<2; i++)
                    for (lc=0; lc<output->atm.nlev-1; lc++)
                      status += arb_wvn(output->wl.nlambda_t, output->wl.lambda_t, output->jacobian_t[lu][is][js][isp][i][lc], 
                                        output->wl.nlambda_h, output->wl.lambda_h, output->jacobian[lu][is][js][isp][i][lc], linear, 0);
...
int multiply_extraterrestrial (input_struct input, output_struct *output)
{
  ...
  for (iv=0; iv<output->wl.nlambda_h; iv++) {
    ...
    status = scale_output (input,
                           ...
                           &(output->uavgdn3d), &(output->uavgup3d), &(output->radiance3d),
                           &(output->jacobian), &(output->absback3d), 
                           ...
                           output->atm.nzout, output->atm.Nxcld, output->atm.Nycld, output->atm.Nzcld, output->mc.alis.Nc, output->atm.nlyr,
                           output->atm.threed, 
                           output->mc.sample.passback3D,
                           output->islower, output->isupper, output->jslower, output->jsupper, 
                           &(output->abs3d),
...
static int write_spectral3D (input_struct input, output_struct *output)
{
  ...
  int is=0, js=0, ks=0, iv=0, ip=0, ic=0, isp=0, kc=0;
  ...
  FILE *fflx=NULL, *fflxvar=NULL, *fabs=NULL, *fabsvar=NULL, *frad=NULL, *fradvar=NULL, *fjac=NULL;  /* 27.02.2013 **CK **BM: add *fabsvar=NULL for thermal backward heating rates std */
	  ...
	  output->uavgup3d   [ks][is][js][0] *= ffactor;
          ...
          if (input.rte.mc.jacobian)
            for (isp=0; isp<input.n_caoth+2; isp++)
              for (i=0; i<2; i++)
                for (lc=0; lc<output->atm.nlev-1; lc++)
                  output->jacobian [ks][is][js][isp][i][lc][0]*=rfactor; //CE: this should probably be ffactor if Jacobian for diffuse irradiance is calculated.
            ...
            for (i=0; i<2; i++)
              for (lc=0; lc<output->atm.nlev-1; lc++)
                output->jacobian [ks][is][js][isp][i][lc][0] =
                  integrate_float (xint, 
                                   output->jacobian [ks][is][js][isp][i][lc], 
                                   output->wl.nlambda_h); 
        
        
        if (input.rte.mc.backward.absorption) 
          output->absback3d  [ks][is][js][0] =
            integrate_float (xint, 
  ...
  if (input.source != SRC_THERMAL) {
    ...
    if (output->wl.nlambda_h>0) {
      ...
      status = scale_output (input,
                             ...
                             &(output->rfldir3d_var), &(output->rfldn3d_var), &(output->flup3d_var), &(output->uavgso3d_var),
                             &(output->uavgdn3d_var), &(output->uavgup3d_var), &(output->radiance3d_var),  &(output->abs3d_var), &(output->absback3d_var), 
                             output->atm.nzout, output->atm.Nxcld, output->atm.Nycld, output->atm.Nzcld, output->mc.alis.Nc, output->atm.nlyr, output->atm.threed, 
                             output->mc.sample.passback3D,
                             output->islower, output->isupper, output->jslower, output->jsupper, 
                             &(output->abs3d),
...
int scale_output (input_struct input,
                  ...
                  float *****p_uavgdn3d, float *****p_uavgup3d, float *******p_radiance3d, float ********p_jacobian, float *****p_absback3d, 
                  float *****p_rfldir3d_var, float *****p_rfldn3d_var, float *****p_flup3d_var, float *****p_uavgso3d_var,
                  float *****p_uavgdn3d_var, float *****p_uavgup3d_var, float ******p_radiance3d_var,
                  float *****p_abs3d_var, float *****p_absback3d_var, 
                  int nzout, int Nx, int Ny, int Nz, int Nc, int Nlyr, int *threed, int passback3D,
