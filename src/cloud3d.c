/*--------------------------------------------------------------------
 * $Id: cloud3d.c 3512 2019-11-23 10:10:01Z bernhard.mayer $
 * 
...
int setup_atmosphere3D (input_struct input, output_struct *output)
{
  ...
  case MC_SENSORPOSITION_CARTESIAN:
    ...
    int shifted=0;
    while (output->mc.sample.sensorposition_x[0] < 0) {
      output->mc.sample.sensorposition_x[0] += output->mc.sample.Nx * output->mc.sample.delX;
      shifted++;
    }
    
    while (output->mc.sample.sensorposition_x[0] >= output->mc.sample.Nx * output->mc.sample.delX) {
      output->mc.sample.sensorposition_x[0] -= output->mc.sample.Nx * output->mc.sample.delX;
      shifted++;
    }
    
    while (output->mc.sample.sensorposition_x[1] < 0) {
      output->mc.sample.sensorposition_x[1] += output->mc.sample.Ny * output->mc.sample.delY;
      shifted++;
    }

    while (output->mc.sample.sensorposition_x[1] >= output->mc.sample.Ny * output->mc.sample.delY) {
      output->mc.sample.sensorposition_x[1] -= output->mc.sample.Ny * output->mc.sample.delY;
      shifted++;
    }

    if (!input.quiet && shifted>0)
      fprintf (stderr, " ... shifting sensorposition perioducally to %f %f\n",
	       output->mc.sample.sensorposition_x[0], output->mc.sample.sensorposition_x[1]);
  ...
  /* setup triangular surface */

  if (strlen(input.rte.mc.filename[FN_MC_TRIANGULAR_SURFACE])>0) {

    setup_triangular_surface(input.rte.mc.filename[FN_MC_TRIANGULAR_SURFACE], &(output->mc.triangular_surface), input.quiet);/* masc */
