...
c  ---------------------------------------------------------------------
   15 continue
      ...
      do i = 1, nv1
         fkg(i) = fkga(i)/330.0*umco2 + pq(i)*fkgb(i)
cbm Had to add this check because the above line over-corrects 
cbm for too small CO2 concentrations and provides negative fkg()
         if (fkg(i) .lt. 0) then
            fkg(i) = 0.0
            WRITE (0,*) 'Warning, CO2 mixing ratio',
     1            umco2,'ppm out of range in Fu band 15!'
         endif
      end do
      call qophc (fkg, tg)
      hk = hk15(ig)
