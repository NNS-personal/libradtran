/*--------------------------------------------------------------------
 * $Id: mie.c 3456 2019-02-28 15:28:40Z Claudia.Emde $
 * 
...
int setup_alpha(/* Output */
		float** alpha, int* nalpha,
		/* Input */
		char* alpha_filename, float input_alpha, int verbose);
...
int mie (input_struct input, output_struct *output)
{
  ...
  status = setup_alpha (&(output->alpha), &(output->nalpha),
			input.alpha_filename, input.alpha, input.verbose);
  ...
  for (iv=lambda_i_start; iv<=lambda_i_end; iv++) {
	...
	if (input.distribution != DIST_AER){
	  ...
	  status = calc_distribution_number_dens (input.distribution,
						  output->alpha[ia], output->r_eff[ir], 
	...
	if(input.output_format != OUTPUT_FORMAT_NETCDF){
	  ...
	  if (iv == 0  && ir ==0){
	    ...
	    status=create_netcdf_file(input.basename, input.medium, input.aerosol_type, 
				      input.distribution, output->alpha[ia],  
				      output->wl.nlambda, output->wl.lambda,
	...
	if ((ir==output->n_r_eff-1 && ia==output->nalpha-1)
...
int setup_alpha(float** alpha, int* nalpha,
		char* alpha_filename, float input_alpha, int verbose)
{
  int status = 0, ia=0;
