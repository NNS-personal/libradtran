/*--------------------------------------------------------------------
 * $Id: molecular.c 3453 2019-02-03 22:21:33Z josef.gasteiger $
 * 
                         ...
                         int **molabs_src)
{
  ...
  if (levelsum (&(output->atm), MOL_O4) > 0) {
    status = crs_o4 (&output->crs, first, input.ck_abs[CK_ABS_O4],
                     ...
                     output->atm.microphys.temper, output->atm.nlev,
		     output->atm.Nxatm, output->atm.Nyatm,
                     input.filename[FN_PATH], input.crs_model[CRS_MOL_O4]);
  ...
  case CRS_MODEL_VANDAELE:
	...
	for (ix=0; ix<Nxatm; ix++){
	  for (iy=0; iy<Nyatm; iy++){
		...
		a= (coeff_crs[1][ia] - coeff_crs[0][ia]) / (294.-220.) *
		  (temper[ix][iy][it] - 220.) +  coeff_crs[0][ia];
		
		if (ia == ib) {
		  crs_out->crs[ix][iy][it][MOL_NO2][iv] = a*fact;
		}
		else {
		  b = (coeff_crs[1][ib] - coeff_crs[0][ib]) / (294.-220.) *
		    (temper[ix][iy][it] - 220.) +  coeff_crs[0][ib];
		  gradient=(a-b)/(lambda_raw[ia]-lambda_raw[ib]);
