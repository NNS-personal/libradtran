/*--------------------------------------------------------------------
 * $Id: solve_rte.c 3524 2019-12-23 01:49:09Z bernhard.mayer $
 * 
...
static int reset_rte_output (rte_output **rte, input_struct input,
                             int nzout, 
                             int Nxcld, int Nycld, int Nzcld, 
                             int Nxsample, int Nysample,
                             int Ncsample, int Nlambda, 
...
static void free_rte_output (rte_output *result, input_struct input,
                             int nzout,
                             int Nxcld, int Nycld, int Nzcld, 
                             int Nxsample, int Nysample,
                             int Ncsample, int Nlyr, int Nlambda,
                             int *threed, int passback3D);

static int add_rte_output (rte_output *rte, rte_output *add, double factor, double* factor_spectral, 
                           input_struct input, 
                           int nzout, int Nxcld, int Nycld, int Nzcld, int Nc, int Nlyr, int Nlambda,  
                           int *threed, int passback3D,
      ...
      if (input.rte.solver == SOLVER_MONTECARLO){
        ...
        */ 
        if (input.rte.mc.core_isactive) {
          ...
          float *** core;
          float *** distances;
          if(!(distances = calloc_float_3D(nz, nx, ny, "distances"))) return -1;
          ...
          if(input.rte.mc.core_inputfile){/*If the user specified the cloud core*/
            ...
            float* core_dz;
            status=read_3D_caoth_header(input.rte.mc.core_inputfile, &core_nx,&core_ny,&core_nz,&core_flag, &core_dx, &core_dy, &core_dz,&core_rows);
            if(status!=0){ return -1;}
            if(core_flag!=4)
              {fprintf(stderr, "!!Warning: when reading %s, another flag than 4 was found! Is this really a file describing a cloud core?\n", input.rte.mc.core_inputfile);}
            core_dx*=1000;
            core_dy*=1000;
            if(nx!=core_nx||ny!=core_ny||dx!=core_dx||dy!=core_dy)
              {fprintf(stderr, "Error when reading core profile %s: The horizontal grid of the core must be the same as in wc_file 3D!\n", input.rte.mc.core_inputfile); return -1;}
            float*** column4;
            int* core_threed=calloc(core_nz, sizeof(int));
            if(!(column4 = calloc_float_3D(core_nz, core_nx, core_ny, "column4"))) return -1;
            int *indz=calloc(core_rows-2, sizeof(int));
            status=read_3D_caoth_data(input.rte.mc.core_inputfile,core_nx, core_ny, core_nz, core_rows, &column4,NULL, NULL, NULL, NULL, indz);
            if(status!=0){ return -1;}
            ...
            if(!(core = calloc_float_3D(nz, nx, ny, "core"))) return -1;
              ...
              for (size_t j = 0; j < k_scat->ny; j++) {
                for (size_t k = 0; k < k_scat->nz; k++) {
                  if((output->caoth3d[isp_wc3D]).threed[k])
                    set3d_f(k_scat, i,j,k, ((output->caoth3d[isp_wc3D]).ext[k][i][j])*((output->caoth3d[isp_wc3D]).ssa[k][i][j]));
                  else
            ...
            for (size_t i = 0; i < k_scat->nz; i++)
              deltaz[i]=1000*(output->atm.zd_common[k_scat->nz-i-1]-output->atm.zd_common[k_scat->nz-i]);//convert to meters
		...
		for (size_t i = 0; i < k_scat->nx; i++) {
		  ...
		  zstart[k*k_scat->ny*k_scat->nx+j*k_scat->nx+i] = z_index[k];
            ...
            arr3d_f* distances_linarray=get_distances2(*k_scat, dx, dy, deltaz, xstart, ystart, zstart,total_starts, input.rte.mc.core_threshold);
            black_white_filter(distances_linarray, input.rte.mc.core_threshold, core_linarray);
            
            for (size_t i = 0; i < nx; i++){
              for (size_t j = 0; j < ny; j++){
                for (size_t k = 0; k < nz; k++){
                  core[k][j][i]=(float)get3d_i(core_linarray, i,j,k);
                  distances[k][j][i]=get3d_f(distances_linarray, i,j,k);
                }
              }
            }
              ...
              for (size_t k = 0; k < nz; k++) {
                if(threed[k]) {
                  if(core[k][i][j]>0.0) {
                    ...
                    (output->caoth3d[isp_wc3D]).ff[k][i][j]=0.0;
                    (output->caoth3d[isp_wc3D]).g1[k][i][j]=0.0;
                    (output->caoth3d[isp_wc3D]).g2[k][i][j]=0.0;
                    (output->caoth3d[isp_wc3D]).ssa[k][i][j]=0.0;
                    ...
                    (output->caoth3d[isp_hiddencore]).ff[k][i][j]=0.0;
                    (output->caoth3d[isp_hiddencore]).g1[k][i][j]=0.0;
                    (output->caoth3d[isp_hiddencore]).g2[k][i][j]=0.0;
                    (output->caoth3d[isp_hiddencore]).ssa[k][i][j]=0.0;
                  }
                }
              }
            }
          ...
          if(!input.quiet)
            fprintf(stderr, "scaled %d pixels!\n", counter);
...
/*	  if (input.tipa==TIPA_DIR) {
	...
	/* reset band integral */
        reset_rte_output(&rte_outband, input, output->atm.nzout,
                         output->atm.Nxcld, output->atm.Nycld,
                         output->atm.Nzcld,
                         output->mc.sample.Nx, output->mc.sample.Ny,
                         output->mc.alis.Nc, output->mc.alis.nlambda_abs,
                         output->atm.nlev-1,
	    ...
	    if ( input.raman ) {
	      if      ( ir == 0 )  weight = 0;
            ...
            add_rte_output (rte_outband, rte_out, weight, weight_spectral, input, output->atm.nzout, 
			    output->atm.Nxcld, output->atm.Nycld, output->atm.Nzcld, 
			    output->mc.alis.Nc, output->atm.nlev-1,  output->mc.alis.nlambda_abs,
                            output->atm.threed, output->mc.sample.passback3D,
	  ...
	  for(lu=0; lu<output->atm.nzout; lu++) {
		...
		for (js=output->jslower; js<=output->jsupper; js++) {
		  ...
		  output->uavgso3d_r   [lu][is][js][iv] += 
		    output->ipaweight[ipa] * rte_outband->uavgso3d   [
                                                                      lu][is][js];
		  ...
		  output->uavgup3d_r   [lu][is][js][iv] += 
		    output->ipaweight[ipa] * rte_outband->uavgup3d   [lu][is][js];
                  ...
                  if(output->mc.sample.abs_jacobian){
	...
	return -1;
      ...
      status = scale_output (input,
                             ...
                             &(output->uavgdn3d_r), &(output->uavgup3d_r), &(output->radiance3d_r),
                             &(output->jacobian_r), &(output->absback3d_r), 
                             &(output->rfldir3d_var_r), &(output->rfldn3d_var_r), &(output->flup3d_var_r), &(output->uavgso3d_var_r),
                             &(output->uavgdn3d_var_r), &(output->uavgup3d_var_r), &(output->radiance3d_var_r),
                             &(output->abs3d_var_r), &(output->absback3d_var_r), 
                             output->atm.nzout, output->atm.Nxcld, output->atm.Nycld, output->atm.Nzcld, output->mc.alis.Nc,
                             output->atm.nlev-1, output->atm.threed, 
  ...
  free_rte_output (rte_out, input,  output->atm.nzout,  
		   output->atm.Nxcld, output->atm.Nycld, output->atm.Nzcld, 
		   output->mc.sample.Nx, output->mc.sample.Ny, 
                   output->mc.alis.Nc, output->atm.nlyr-1, output->mc.alis.nlambda_abs,
		   output->atm.threed, output->mc.sample.passback3D);
                  ...
                  double **p_rho_mass_zout, float **p_k_abs_layer, float **p_k_abs, float **p_k_abs_outband)
{
  int status=0;
  int nlambda=0;
  int lc=0, lu=0, is=0, js=0, ip=0, ic=0, isp=0, i=0;
  ...
  if (output->mc.sample.passback3D) {
    ...
    if (input.rte.mc.jacobian)
      output->jacobian_r = calloc (output->atm.nzout, sizeof(float ******));
    ...
    for (lu=0; lu<output->atm.nzout; lu++) {
      ...
      if (input.rte.mc.jacobian)
        output->jacobian_r [lu] = calloc (output->mc.sample.Nx, sizeof (float *****));
	...
	output->radiance3d_r [lu][is] = calloc (output->mc.sample.Ny, sizeof (float ***));
        ...
        if (input.rte.mc.jacobian)
          output->jacobian_r [lu][is] = calloc (output->mc.sample.Ny, sizeof (float ****));
	...
	for (js=output->jslower; js<=output->jsupper; js++) {
	  ...
	  output->radiance3d_r [lu][is][js] = calloc (input.rte.mc.nstokes, sizeof (float **));
	    ...
	    for(ic=0; ic<output->mc.alis.Nc; ic++)
          ...
          if (input.rte.mc.jacobian){
            output->jacobian_r [lu][is][js] = calloc ( input.n_caoth+2, sizeof (float ***));
            for (isp=0; isp<input.n_caoth+2; isp++){
              output->jacobian_r [lu][is][js][isp] = calloc ( 2, sizeof (float **));
              for (i=0; i<2; i++){
                output->jacobian_r [lu][is][js][isp][i] = calloc ( output->atm.nlev-1, sizeof (float *));
                
                for (lc=0; lc<output->atm.nlev-1; lc++){
                  output->jacobian_r [lu][is][js][isp][i][lc] = calloc ( output->wl.nlambda_r, sizeof (float));
                }
              }
            }
          }
  ...
  if (output->mc.sample.passback3D) {
    ...
    if (input.rte.mc.jacobian)
      output->jacobian = calloc (output->atm.nzout, sizeof (float ******));
    ...
    for (lu=0; lu<output->atm.nzout; lu++) {
      ...
      if (input.rte.mc.jacobian)
        output->jacobian [lu] = calloc (output->mc.sample.Nx, sizeof (float *****));
	...
	output->radiance3d [lu][is] = calloc (output->mc.sample.Ny, sizeof (float ***));
        ...
        if (input.rte.mc.jacobian)
          output->jacobian [lu][is] = calloc (output->mc.sample.Ny, sizeof (float ****)); 
	...
	for (js=output->jslower; js<=output->jsupper; js++) {
	  ...
	  output->uavgup3d   [lu][is][js] = calloc (nlambda, sizeof (float));
          ...
          if (input.rte.mc.jacobian){
            output->jacobian [lu][is][js] = calloc ( input.n_caoth+2, sizeof (float ***));
            for (isp=0; isp<input.n_caoth+2; isp++){
              output->jacobian [lu][is][js][isp] = calloc ( 2, sizeof (float **));
              for (i=0; i<2; i++){
                output->jacobian [lu][is][js][isp][i] = calloc ( output->atm.nlev-1, sizeof (float *));
                
                for (lc=0; lc<output->atm.nlev-1; lc++){
                  output->jacobian [lu][is][js][isp][i][lc] = calloc ( nlambda, sizeof (float));
                }
              }
            }
          }
				      ...
				      int *threed, int passback3D)
{
  ...
  if (passback3D) {
    ...
    if(input.rte.mc.jacobian){
      ...
      status += ASCII_calloc_float_6D (&(result->jacobian), nzout,
                                       Nxsample, Nysample,
                                       input.n_caoth+2, 2, Nlyr);
    }
...
static int reset_rte_output (rte_output **rte, input_struct input,
                             ...
                             int *threed, int passback3D) 
{
  free_rte_output(*rte, input, nzout, Nxcld, Nycld, Nzcld, Nxsample, Nysample, Ncsample, Nlyr, Nlambda, 
                  threed, passback3D);
  *rte = calloc_rte_output (input, nzout, Nxcld, Nycld, Nzcld,
                            Nxsample, Nysample, Ncsample, Nlambda,
                            Nlyr, 
                            threed, passback3D);
...
static int add_rte_output (rte_output *rte, rte_output *add, double factor, double* factor_spectral,
			   ...
			   int islower, int isupper, int jslower, int jsupper)
{
  int j=0, iu=0, is=0, js=0, ks=0, lu=0, ip=0, ic=0, lc=0, iv_alis;
  int isp=0, ijac=0;
  double factor2=0;
          ...
          if(input.rte.mc.jacobian){
            for (isp=0; isp<input.n_caoth+2; isp++) {
              for (ijac=0; ijac<2; ijac++) {
                /* scattering and absorption */
                for (lc=0; lc< Nlyr; lc++) {
                  rte->jacobian [ks][is][js][isp][ijac][lc] += 
                    factor * add-> jacobian [ks][is][js][isp][ijac][lc];
                }
              }
            }
          }
  ...
  if (passback3D) {
    ...
    if(input.rte.mc.jacobian)
      if (result->jacobian != NULL)
        ASCII_free_float_6D(result->jacobian, 
                            nzout, Nxsample, Nysample,
                            input.n_caoth+2, 2);
  ...
  case SOLVER_MONTECARLO:
	...
	return -1;
      ...
      status = mystic (&output->atm.nlyr,
                       ...
                       output->surfaces.label, &(output->surfaces.n),
	...
	if (!input.rte.mc.backward.yes) {/*TZ bt*/
	  ...
	  /* thermal emission of the atmosphere */
        ...
        status = mystic (&output->atm.nlyr,
                         ...
                         rte_out->uavgso, rte_out->uavgdn, rte_out->uavgup,
                         rte_out->rfldir3d, rte_out->rfldn3d, rte_out->flup3d, 
                         rte_out->uavgso3d, rte_out->uavgdn3d,
                         rte_out->uavgup3d, 
                         rte_out->radiance3d,
                         rte_out->absback3d, 
                         rte_out->abs3d,
                         rte_out->radiance3d_is,
                         rte_out->jacobian,
                         rte_out->rfldir3d_var, rte_out->rfldn3d_var,
                         rte_out->flup3d_var, 
                         rte_out->uavgso3d_var,
                         rte_out->uavgdn3d_var,
                         rte_out->uavgup3d_var, 
        ...
        if (!input.rte.mc.backward.yes && !input.rte.mc.tenstream && !input.rte.mc.nca) {/*TZ bt, no separate surface emission needed*/ /* Carolin Klinger 2019 */
          if ( ib == 0 && !input.quiet )
